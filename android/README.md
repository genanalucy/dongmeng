...
1. 未登录进入“账户与权益”默认显示“手机号登录”，字段仅为“手机号”和“密码”；可通过“注册账户”切换到独立注册表单，注册字段仅为“用户名”“手机号”“密码”“确认密码”。界面不显示或提交邮箱。
2. 所有字段有中文无障碍标签，密码字段隐藏输入内容、使用密码键盘和合理 IME；输入框及操作控件触达区域至少 48dp。
3. 本地策略将有效中国大陆手机号规范为 `+86` 格式。无效登录或注册输入不会派发请求；注册还校验用户名、强密码和确认密码。错误提示为固定中文文本，不插入手机号、密码或服务端原始错误。
4. 有效注册调用 `AccountViewModel.register(username, phone, password)`；该路径会自动登录并拉取用户和权益。有效登录调用 `AccountViewModel.login(phone, password)`。认证流程不调用验证码端点。
5. 重复号码、网络或服务端错误仅显示固定安全提示“账户状态暂时无法更新，请稍后重试。”；不得截图或记录完整手机号、密码、令牌或 DSN。
