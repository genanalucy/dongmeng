...
1. 未登录进入“账户与权益”默认显示邮箱登录，并可通过“注册账户”进入注册表单；注册页仅显示邮箱、密码和确认密码，三个字段均提供中文标签，密码字段隐藏输入内容。
2. 注册按钮仅在邮箱、密码和确认密码通过本地校验且未处理中时可用：邮箱会去除首尾空白并按服务端接受规则检查；密码至少 8 个字符，且包含 ASCII 大写字母、小写字母与数字；确认密码必须完全一致。
3. 以新的有效邮箱提交注册后，客户端仅调用既有 Cloud API 注册接口。服务端会在一个数据库事务中创建用户及 3 天试用权益；随后既有 `AccountViewModel.register` 自动登录、拉取用户和权益，并进入原有已登录导航状态。
4. 重复邮箱、网络或服务端错误只显示固定安全提示“账户状态暂时无法更新，请稍后重试。”，不得在账户摘要或表单中显示邮箱、密码、令牌或原始后端错误。注册流程不包含邮箱验证。
