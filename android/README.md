...
4. 开始讲话至少 20 秒，确认每 8 秒创建一个 Turn，字幕继续滚动，PCM 上行无断包报错。
5. 验证“暂停”停止采集并完成当前 Turn，“恢复”创建新 Turn，“结束”发送 `finish` 并回到空闲。
6. 测试录音中拔出麦克风/耳机、撤销权限、关闭 Agent、锁屏再返回；应用应显示错误而不是崩溃。
7. Android 音频路由可能受蓝牙 profile、厂商 DSP 和无障碍声道设置影响；最终验收必须使用目标机型与目标耳机。
