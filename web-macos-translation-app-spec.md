...
产品还提供独立的“单人同声传译”模式：一个麦克风持续采集，按 8 秒 Turn 主动结束并立即创建下一 Session，物理麦克风在 Turn 边界不重启。单人模式支持中文/英文交换、暂停、恢复、结束、实时原译文、复制和 UTF-8 TXT 导出；TTS 可选双耳、左耳、右耳或仅字幕。仅字幕策略在浏览器协议边界消费并丢弃 PCM，不触发播放或 UI PCM 事件，且不要求输出设备。输入麦克风断开、所需输出断开、页面隐藏、失焦或卸载时必须停止采集并取消全部后台 Turn。
