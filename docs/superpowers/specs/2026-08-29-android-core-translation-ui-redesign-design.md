...
- 一条完成消息显示源语言、原文、细分隔线和靛蓝译文；单条重播明确延期：当前行为权威层没有受控 replay API，因此不显示播放按钮，也不改动 ViewModel、Coordinator 或音频管线；
...
- 所有触摸目标最小 48dp；
- 语言 chip、左右话筒、暂停、结束和底部导航必须可键盘/无障碍服务操作；单条重播延期，待行为权威层提供受控 API 后另行验收；
- 靛蓝文字/背景组合满足正文对比度；
...
10. 单条重播延期：在 ViewModel/Coordinator 提供受控 replay API 前不显示播放按钮，且不得改变既有音频管线；
11. 重新构建 Debug APK，但 APK、Gradle/Kotlin 缓存均不进 Git。
